...
function commands:Add(name,alias,desc,exec)
    ...
    self.command_list[name]={name=name,alias=type(alias)=='table'and alias or {'None'},desc=type(desc)=='string'and desc or ('No description provided'),exec=type(exec)=='function'and exec or function(message) return message:reply('No command asigned') end}
