...
commands:Add('yt',{},"wip", function(message,args,t)
    if args[1] then
        if args[1]:match('[?&]v=([^&]*)') then
            local id=args[1]:match('[?&]v=([^&]*)')
            message:reply('Attempting to download song ID='..id)
            local s=io.popen('ytdl.exe -x --audio-format mp3 --output "./tmp/'..id..'.%(ext)s" https://www.youtube.com/watch?v='..id)
            if s then
                local final=''
                repeat
                    local c=s:read(1)
                ...
                until not c
                message:reply({file='./tmp/'..id..'.mp3'})
                os.remove('./tmp/'..id..'.mp3')
            else
